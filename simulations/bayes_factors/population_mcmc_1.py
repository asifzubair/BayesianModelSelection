...
##        -1.03496617,  3.83181641, -3.47513347,  1.32579626,  3.43152853]) 
    ...
    log_likelihood0 = np.array([calculate_log_likelihood_1(data, param) for param in theta0])
    log_likelihood = temps * log_likelihood0
    log_prior = np.array([calculate_log_prior_1(param) for param in theta0])
    ...
    for i in range(num_steps):
        ...
        theta = np.array([reflect_proposal_1(t, prior_min, prior_max) for t in theta])
        proposed_log_likelihood0 = np.array([calculate_log_likelihood_1(data, param) for param in theta])
        proposed_log_likelihood = temps * proposed_log_likelihood0
        proposed_log_prior = np.array([calculate_log_prior_1(param) for param in theta])
